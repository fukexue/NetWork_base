...
def conv(input_array, kernel_array, output_array, stride, bias):
    output_weight = output_array.shape[1]
    output_height = output_array.shape[0]
    kernel_weight = kernel_array.shape[-1]
...
class ConvLayer(object):
    def __init__(self, input_width, input_height, channel_number,
        ...
        self.filter_heigth = filter_height
    ...
    def expand_sensitivity_map(self, sensitivity_array):
        ...
        expand_height = (self.input_height - self.filter_heigth + 2*self.zero_padding+1)
    ...
    def bp_sensitivity_map(self, sensitivity_array, activator):
        ...
        expand_array_width = expand_array.shape[2]
        zp = (self.input_width + self.filter_width - 1 - expand_array_width)//2
        padded_array = padding(expand_array, zp)
        self.delta_array = self.create_delta_array()
        for f in range(self.filter_number):
            # filter在这里相当于一个四维空间
            filter = self.filters[f]
            flipped_weigths = np.array(list(map(lambda i: np.rot90(i, 2), filter.get_weight())))
            delta_array = self.create_delta_array()
            for d in range(delta_array.shape[0]):
                conv(padded_array[f], flipped_weigths[d], delta_array[d], 1, 0)
